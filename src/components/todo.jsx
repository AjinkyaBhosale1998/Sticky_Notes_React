...
const Todo = () => {
  ...
  const [todos, setTodos] = useState ([
    { id: uuidv4(), todo: "Hi 👋🏻 my name is Ajinkya & I'm a passionate Frontend Web Developer from Pune, ⚡ Expertise in React JS , Angular , JavaScript / TypeScript , HTML , CSS ." ,
    isCompleted: false },
  ]);
  ...
  <div>
    ...
    <div className="container mx-auto my-5 rounded-xl p-5 bg-violet-100 min-h-[80vh] w-1/2">
      <div className="addTodo flex items-center">
        ...
        <button
          onClick={handleAdd}
          className="bg-violet-800 hover:bg-violet-950 p-3 py-1 text-white rounded-md text-xl"
        >
          Add
        </button>
      </div>
      <h2 className="text-4xl font-bold mt-4">Your Todos ✅</h2>
      <div className="todos">
        {todos.map((item) => (
          <div
          ...
          >
            ...
            />
            <div
              className={`flex-grow ${item.isCompleted ? "line-through" : ""
                }`}
            >
              {item.todo}
            </div>
